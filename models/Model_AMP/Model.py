        ...
        for model, filename in io.progress_bar_generator(self.model_filename_list, "Initializing models"):
            ...
            if self.is_training and gan_power != 0 and model == self.GAN:
                if self.gan_model_changed:
                    do_init = True
            if not do_init:
                do_init = not model.load_weights_jmode( self.get_strpath_storage_for_file(filename) )
#k_model_load   do_init = not model.load_weights( self.get_strpath_storage_for_file(filename) )
        ...
        if self.is_training:
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, pak_name=self.src_pak_name, ignore_same_path=ignore_same_path, random_ct_samples_path=random_ct_samples_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
                    ...
                    SampleGeneratorFace(training_data_dst_path, pak_name=self.src_pak_name, ignore_same_path=ignore_same_path, debug=self.is_debug(), batch_size=self.get_batch_size(),
    ...
    def onSave(self):
        for model, filename in io.progress_bar_generator(self.get_model_filename_list(), "Saving", leave=False):
            model.save_weights_jmode ( self.get_strpath_storage_for_file(filename))
#k_model_save   model.save_weights ( self.get_strpath_storage_for_file(filename) )
