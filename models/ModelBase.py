...
import utils.globalvar as gl #kingboy global var
...
#k_model_load
            saved_models_k= gl.get_value('saved_models_global')  #kingboy global var
            if saved_models_k !=None:
               io.log_info ("Update model data:"+saved_models_k)
            if saved_models_k == 'readjob-savejob' or saved_models_k == 'readjob-savepick':
                 with open(self.model_data_path, 'rb') as fo:
                      model_data =joblib.load(fo)
            elif saved_models_k == 'readjob-savepick-model':
                 with open(self.model_data_path, 'rb') as fo:
                      model_data =joblib.load(fo) 
                 pathex.write_bytes_safe (Path( self.get_strpath_storage_for_file('data.dat.pick') ), pickle.dumps(model_data) )	                               
            else:
    ...
    def ask_override(self):
        time_delay = 5 if io.is_colab() else 3
...
#k_model_saved
        ...
        if saved_models_k == 'readjob-savejob' or saved_models_k == 'readpick-savejob':
             pathex.write_bytes_safe_j (self.model_data_path, model_data )
        else:
    ...
    def get_summary_text(self):
        ...
        summary_text += [f'=={" RG 2.5 version was edited by kingboy! ":-^{width_total}}==']
        summary_text += [f'  {" "*width_total}  ']
        summary_text = "\n".join (summary_text)
        return summary_text
