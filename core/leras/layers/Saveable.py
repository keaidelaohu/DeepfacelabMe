...
#k_saved_weights 2022.3.08
    def save_weights_jmode(self, filename, force_dtype=None):
        d = {}
        weights = self.get_weights()
...
#k_saved_models_filelist

        saved_models = gl.get_value('saved_models_global')  #kingboy global var
        if saved_models == 'readjob-savejob' or saved_models == 'readpick-savejob' :
             pathex.write_bytes_safe_j ( Path(filename), d )
        else:
             d_dumped = pickle.dumps (d, 4)
             pathex.write_bytes_safe ( Path(filename), d_dumped )		
		

    def load_weights_jmode(self, filename):
        """
        returns True if file exists
        """
        filepath = Path(filename)
...
#k_load_models
            saved_models = gl.get_value('saved_models_global')  #kingboy global var
            if saved_models == 'readjob-savejob' or saved_models == 'readjob-savepick':
               with open(filepath, 'rb') as fo:
                   d = joblib.load(fo)
            elif saved_models == 'readjob-savepick-model':
               with open(filepath, 'rb') as fo:
                   d = joblib.load(fo)
               d_dumped = pickle.dumps (d, 4)
               pathex.write_bytes_safe ( Path(filename+'.pick'), d_dumped )
