...
def trainerThread (s2c, c2s, e,
    ...
    while True:
        try:
            ...
            training_time_state=kwargs.get('training_time', 0) #K Training Auto Save
            auto_shutdown = False
                ...
                if not debug:
                    ...
                    if not is_reached_goal:
                        ...
                        if iter_time >= 10:
                            ...
                            loss_string = "{0}[#{1:06d}][{2:04d}ms]".format(time_str, iter, int(iter_time * 900))
                        ...
                        #K Training Auto Save
                        if training_time_state !=0 and training_time_state<= (cur_time - start_time):
                            io.log_info('Timing duration of model training (%d minutes).'% (training_time_state//60))
                            model_save()
                            is_reached_goal = True
                            if auto_shutdown:
                                io.log_info('The model has been saved. Now try turning off your computer...')
                                time.sleep(3)
                                os.system("shutdown /s /t 1")
                            else:
                                io.log_info('The model has been saved.')
                            break
